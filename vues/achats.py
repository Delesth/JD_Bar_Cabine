...
from core.format import afficher_flash, date_fr, fcfa, flash, gen, nombre, pluriel, vider
...
def page():
    ...
    jour = st.date_input("Date de l'achat", value=date.today(), max_value=date.today(), format="DD/MM/YYYY",
                         key=f"a{g}_date")
    p = st.selectbox("Produit", produits, format_func=lambda x: x.nom, key=f"a{g}_produit")
    c1, c2 = st.columns(2)
    cond = c1.selectbox("Acheté en", list(CONDITIONNEMENTS), format_func=CONDITIONNEMENTS.get,
                        index=list(CONDITIONNEMENTS).index(p.conditionnement), key=f"a{g}_cond_{p.id}")
    if cond == "unité":
        upc = 1
        c2.text_input("Unités par conditionnement", value="1", disabled=True, key=f"a{g}_upc_dis_{p.id}")
    else:
        upc = c2.number_input(f"{pluriel(p.unite_vente).capitalize()} par {cond}", min_value=1, step=1,
                              value=int(p.unites_par_cond), key=f"a{g}_upc_{p.id}_{cond}")
